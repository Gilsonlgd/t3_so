...
{
  // verificação de prontidão feita anteriormente pelo S.O
  // filtra tipo de dispositivo virtual e faz a leitura.
  if (dispositivo < 200) {
    ...
    void *contr = self->dispositivo[dispositivo].contr;
    int id = self->dispositivo[dispositivo].id;
    return self->dispositivo[dispositivo].f_escr(contr, id, *pvalor);
  }
...
{
  // filtragem do tipo de dispositivo
  if (dispositivo >= 200) {
    dispositivo = dispositivo - 200;
